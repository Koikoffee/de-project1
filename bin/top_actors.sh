...
OUTFILE="${2:-$PROJECT_ROOT/out/top10_actors.csv}"
TOP_K="${TOP_K:-10}"
WITH_BOM="${WITH_BOM:-0}"
...
gawk -v K="$TOP_K" '
...

  {
    ...
    if(!seen){
      seen=1; hnf=NF; ccol=0
      for(i=1;i<=NF;i++){t=$i; gsub(/^"|"$/,"",t); if(tolower(t)=="cast") ccol=i}
      if(!ccol){print "ERROR: cast column not found" > "/dev/stderr"; exit 2}
      next
    }
